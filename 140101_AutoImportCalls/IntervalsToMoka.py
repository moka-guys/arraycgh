...
Actual usage:    
    ...
    This calls this python script which then reformats the data in the interval-based report 
    generated by genomic workbench, adds sampleID/hybID information from Moka, and creates an
...
cnxn = pyodbc.connect("DRIVER={{SQL Server}}; SERVER={moka_server}; DATABASE={moka_db_name};".format(
        moka_server=config.get("MOKA", "SERVER"),
        moka_db_name=config.get("MOKA", "DATABASE")    
    )
)
cursor = cnxn.cursor()
...
##HEADER changed to 17 as additional header row added when using GC normalisation
